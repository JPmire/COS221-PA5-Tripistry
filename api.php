//copied from katlego's api but my cases added
<?php
...
class TripistryAPI {
    ...
    private function bookPackage($data) {
        $required = ['traveller_id', 'package_id', 'trip_date_id', 'party_size'];
        ...
        try {
            $priceStmt = $this->pdo->prepare("SELECT BasePrice FROM TravelPackage WHERE PackageID = ?");
            $priceStmt->execute([(int)$data['package_id']]);
            $basePrice = $priceStmt->fetchColumn();

            if (!$basePrice) {
                $this->sendResponse("error", "Invalid Package ID.", 404);
            }

            $totalAmount = $basePrice * (int)$data['party_size'];
            
            // 2. Insert the booking
            $stmt = $this->pdo->prepare("
                INSERT INTO Booking (TravellerID, Trip_PackageID, Trip_TripDateID, PartySize, TotalAmount, PaymentStatus) 
                VALUES (?, ?, ?, ?, ?, 'Pending')
            ");
    ...
    private function submitReview($data) {
        if (empty($data['traveller_id']) || empty($data['rating']) || empty($data['comment'])) {
            $this->sendResponse("error", "Missing required fields.", 400);
        }

        $targetAgency = !empty($data['agency_id']) ? (int)$data['agency_id'] : null;
        $targetPackage = !empty($data['package_id']) ? (int)$data['package_id'] : null;

        if (($targetAgency && $targetPackage) || (!$targetAgency && !$targetPackage)) {
            $this->sendResponse("error", "Review must target exactly one Agency OR one Package.", 400);
        }

        try {
            $idStmt = $this->pdo->prepare("SELECT IFNULL(MAX(ReviewID), 0) + 1 FROM Review WHERE TravellerID = ?");
            $idStmt->execute([(int)$data['traveller_id']]);
            $nextReviewID = $idStmt->fetchColumn();
            $stmt = $this->pdo->prepare("
                INSERT INTO Review (TravellerID, ReviewID, Rating, Comment, TargetAgencyID, TargetPackageID) 
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                (int)$data['traveller_id'],
            ...
            $this->sendResponse("success", "Review submitted successfully!");
        } catch (\PDOException $e) {
            $this->sendResponse("error", "Failed to submit review: " . $e->getMessage(), 500);
        }
    }
